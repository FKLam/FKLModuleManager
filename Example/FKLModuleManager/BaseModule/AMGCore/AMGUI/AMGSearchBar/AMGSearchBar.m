...
#import "AMGSearchBar.h"

@interface AMGSearchBar() <UITextFieldDelegate> {
    UITextField *_textField;
    UIImageView *_iconView;
    UIImageView *_iconCenterView;
    AMGSearchBarIconAlign _iconAlignTemp;
}

@end

@implementation AMGSearchBar
...
- (void)setIconAlign:(AMGSearchBarIconAlign)iconAlign {
    if ( !_iconAlignTemp ) {
...
- (void)ajustIconWith:(AMGSearchBarIconAlign)iocnAlign {
    if (_iconAlign == AMGSearchBarIconAlignCenter) {
...
- (void)setPlaceholderColor:(UIColor *)placeholderColor {
    ...
    if ( [[[UIDevice currentDevice] systemVersion] integerValue] < 6 ) {
        [_textField setValue:_placeholderColor forKey:@"_placeholderLabel.textColor"];
    } else {
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    if ( _iconAlignTemp == AMGSearchBarIconAlignCenter ) {
        self.iconAlign = AMGSearchBarIconAlignLeft;
    }
...
{
    if(_iconAlignTemp == AMGSearchBarIconAlignCenter){
        self.iconAlign = AMGSearchBarIconAlignCenter;
    }
